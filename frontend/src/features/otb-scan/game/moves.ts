// Ported from CameraChessWeb (src/utils/moves.tsx).
// Enumerates every legal move (and every legal *pair* of plies) for the
// current position, annotated with the squares that should empty / fill and
// the piece label expected on each destination. The probabilistic matcher
// scores the live board state against these expectations.
...
import { SQUARE_MAP, LABEL_MAP, SQUARE_NAMES } from '../cv/constants'
...
const castlingMap: Record<string, number[]> = {
...
const getData = (pos: Position, move: Move): MovesData | null => {
  ...
  // En passant: the captured pawn vacates a third square.
  else if (
    piece?.role === 'pawn' &&
    move.to !== move.from &&
    !pos.board.get(move.to) &&
...
const combineData = (move1Data: MovesData, move2Data: MovesData): MovesData => {
  ...
  for (let i = 0; i < move1Data.to.length; i++) {
    if (badSquares.includes(move1Data.to[i])) continue
  ...
  return {
    ...
    from: from1.concat(move2Data.from),
    to: to1.concat(move2Data.to),
    targets: targets1.concat(move2Data.targets),
  }
}

// All legal single moves, paired with each legal reply (so a fast two-ply
// exchange can be recognised even if we never saw the intermediate frame).
export const getMovesPairs = (pos: Position): MovesPair[] => {
  ...
  for (const move1 of legalMoves(pos)) {
    ...
    for (const move2 of legalMoves(pos2)) {
      ...
      movesPairs.push({
        move1: move1Data,
        move2: move2Data,
        moves: combineData(move1Data, move2Data),
      })
      done = false
    }

    if (done) {
      movesPairs.push({ move1: move1Data, move2: null, moves: null })
    }
  }
